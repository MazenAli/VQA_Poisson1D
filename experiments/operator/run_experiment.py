...
# %% prepare operator circuits
#
def prepare_operator_circuits(args, operator_driver):
    ...
    verbose_all              = False
    verbose_operator_circuit = False
    
    if 'all' in verbose:
        verbose_all = True
    if 'operator_circuit' in verbose:
        verbose_operator_circuit = True

    # operator circuit
    
    operator_circuits = operator_driver.get_circuits()
        
    # print

    if verbose_all or verbose_operator_circuit:
        print("----------- Operator circuit example ------------------")
        print(operator_circuits[0])
        print("\n")
        print("Total number of operator circuits : ", len(operator_circuits))
        print("----------- -------------------- ------- -----------")
    
    return operator_circuits
...
def save_input(args,
    ...
    target_operator = os.path.join(args.input_dir, 'operator', file_operator)
    ...
    # operator
    with open(target_operator, 'wb') as file:
        pickle.dump(operator_circuits, file)
...
def main():
    ...
    if mode=='submit_experiment':
        ...
        operator_driver   = prepare_operator_driver(args, ansatz_unbound)
        operator_circuits = prepare_operator_circuits(args, operator_driver)
            
        trans_unbound  = transpile(args, device, operator_circuits)
