...
def submit_jobs(device, trans_bound, shots):
    ...
    if num_circs > max_exp and not simulator:
        ...
        num_jobs  = int(num_circs/max_exp)
        remainder = num_circs % max_exp
        if remainder > 0:
            num_jobs += 1
            
        print("Required number of jobs: ", num_jobs)
        
        schedule = [None]*num_jobs
        for j in range(num_jobs):
            start       = j*max_exp
            end         = (j+1)*max_exp
            schedule[j] = slice(start, end)
            
        if remainder > 0:
            start        = max_exp*(num_jobs-1)
            end          = start + remainder
            schedule[-1] = slice(start, end)
    
    # submit jobs
    jobs = []
    for j in range(num_jobs):
        jobs.append(device.run(trans_bound[schedule[j]], experiment_id=experiment_id, shots=shots))
